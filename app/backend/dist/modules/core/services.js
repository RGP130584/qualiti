...
// ==========================================
class CoreService {
    repo = new repositories_1.CoreRepository();
    // ----------------------------------------
    // OCORRÊNCIAS INTELIGENTES (IA-FIRST)
    // ----------------------------------------
    async processRelatarOcorrencia(data) {
    ...
    async listOcorrencias(setor) {
        return await this.repo.listOcorrencias(setor);
    }
    async updateOcorrenciaStatus(id, status, planoCapa, usuario) {
    ...
    async listDocumentos(setor) {
        return await this.repo.listDocumentos(setor);
    }
    // ----------------------------------------
    // AUDITORIA, RISCOS, SEGURANÇA E ANALYTICS
    // ----------------------------------------
    async listAuditorias(setor) { return await this.repo.listAuditorias(setor); }
    async listRiscos(setor) { return await this.repo.listRiscos(setor); }
    async listSeguranca(setor) { return await this.repo.listSeguranca(setor); }
    async getAnalytics() { return await this.repo.getAnalytics(); }
    // ----------------------------------------
    // IA CORPORATIVA: 6 AGENTES INSTITUCIONAIS
    // ----------------------------------------
    async askAiAgent(agente, prompt, usuario, contexto) {
        let resposta = '';
        let acoes_recomendadas = [];
        switch (agente) {
