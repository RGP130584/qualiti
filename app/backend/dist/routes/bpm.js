...
exports.default = bpmRoutes;
const db_1 = __importDefault(require("../db"));
async function bpmRoutes(fastify) {
    ...
    fastify.post('/bpm/fluxos', async (request, reply) => {
        const { nome, descricao, bpmn_json, sla_horas } = request.body;
        const client = await db_1.default.connect();
        try {
    ...
    fastify.post('/bpm/execucoes', async (request, reply) => {
        const { fluxo_id, solicitante } = request.body;
        const client = await db_1.default.connect();
        try {
            ...
            if (bpmn && bpmn.nodes && bpmn.nodes.length > 1) {
                const startNode = bpmn.nodes.find((n) => n.type === 'start');
                if (startNode) {
                    const edge = bpmn.edges.find((e) => e.from === startNode.id);
                    if (edge) {
                        const nextNode = bpmn.nodes.find((n) => n.id === edge.to);
                        if (nextNode)
                            primeiraEtapa = nextNode.label;
                    }
                }
            }
            const logInicial = [{
    ...
    fastify.post('/bpm/execucoes/:id/avancar', async (request, reply) => {
        ...
        const { usuario, proxima_etapa, status_final } = request.body;
        const client = await db_1.default.connect();
        try {
